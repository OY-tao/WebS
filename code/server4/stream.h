...
struct http_awaitable : public stream_base
{
    http_awaitable(task::promise_type *promise, size_t message_size, HttpConn* client)
        : stream_base(promise, message_size),
          client(client) {}
    bool await_ready() { return false; }
    void await_suspend(std::coroutine_handle<task::promise_type> h)
    {
        auto &promise = h.promise();
        this->promise = &promise;
        promise.uring->threadpool_->AddTask(std::bind(&HttpConn::process2,client,h));
    }
    size_t await_resume()
    {
        return promise->res;
...
auto http_conn(HttpConn* client, size_t message_size)
{   
    log("http_conn %lu", message_size);
    return http_awaitable(nullptr, message_size, client);
}
