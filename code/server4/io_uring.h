...
public:
    ...
    void event_loop(task func(int,sockaddr_in));
...
io_uring_handler::io_uring_handler(
    ...
    timer_(new HeapTimer()), threadpool_(new ThreadPool(8))
{

    SqlConnPool::Instance()->Init("localhost", 3306, "root", "123456", "yourdb", 12);
    int isClose_=0;
    if(isClose_) {
...
void io_uring_handler::event_loop(task handle_event(int,sockaddr_in))
{
    // start event loop
    ...
    while (1)
    {
        ...
        io_uring_for_each_cqe(&ring, head, cqe)
        {
            ...
            else if (type == ACCEPT)
            {
                ...
                {
                    connections.emplace(sock_conn_fd, handle_event(sock_conn_fd,(struct sockaddr_in)client_addr));
